...
7. `src/event.rs` — `RingBuffer<256>` with `Event::Keypress`,
   `Event::LineRendered`, `Event::SceneTransition`; chronological
   `iter()` feeds the serial drain
8. `src/serial.rs` — `write_startup_banner` (for release-verify)
...
9. `src/logo.rs` — 128x128 monochrome Shaken Fist logo bitmap,
   generated by `scripts/vendor-logo.py`
10. `src/probes.rs` — language-probe label and status bitmaps for the
    boot-sequence opening beats, generated by
    `scripts/vendor-language-probes.py` from GNU Unifont
11. `scripts/spice.sh` — SPICE-attached QEMU launch script; read this
...
lives at `docs/images/boot-sequence.png` and can be regenerated via
...
Next-milestone open items live in `PLAN-first-playable.md` under
*Future work* (gRPC-over-serial framing, Ryll-side driver, pointer
handshake, mode walk, real audio, scanline overlay).
