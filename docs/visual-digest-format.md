...
- `src/digest.rs` — encoder, all tag constants, and
  `event_tlv_bytes` (the single source of truth for what bytes
  a given event produces on the wire)
- `src/scene.rs::ChannelHashes` — per-channel rolling-hash
  accumulators and the CRC chaining formula
- `src/event.rs` — event variant and mini-enum definitions
- `src/renderer/mod.rs::Renderer::draw_digest` — renderer
- `scripts/digest-payload-smoke.sh` — host-side decoder
...
`src/scene.rs` and its correctness is verified by the chaining
assertions in `scripts/digest-payload-smoke.sh` (an
...
`docs/plans/PLAN-continuous-digest-phase-03-closeout.md` for
verification of ryll's specific behaviour.

## Provenance

- Encoder: `src/digest.rs::encode`
- Event TLV bytes (single source of truth):
  `src/digest.rs::event_tlv_bytes`
- Event variants: `src/event.rs`
- Per-channel rolling hashes: `src/scene.rs::ChannelHashes`
- Renderer: `src/renderer/mod.rs::Renderer::draw_digest`
- Framebuffer hash:
  `src/renderer/mod.rs::Renderer::crc32c_framebuffer_excluding_digest`
