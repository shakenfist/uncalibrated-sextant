...
# keypress via QMP to advance AWAITING -> Booting, waits for the boot
# sequence + parking screen to settle, then issues QMP screendump in
...
# Drive AWAITING -> Booting -> Parked via QMP and snapshot the parking
# screen. Timing: ~1 s AWAITING settle, ~17 boot-script lines at 200 ms
# each ~= 3.4 s, plus a parking-screen beat. 5.5 s post-keypress is
# comfortable.
...
# Synthetic space keypress: advances to Booting.
send('send-key', keys=[{'type': 'qcode', 'data': 'spc'}])
recv()

# Boot sequence plays out; then parking screen settles.
time.sleep(5.5)
